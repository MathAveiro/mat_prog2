...
public class JogoDoGalo {
  ...
  public boolean ultimoJogadorGanhou() {
    ...
    boolean result;
       ...
       // vertical |
    result = contaIgual(ultimaLin, ultimaCol, -1, 0) + 1 + contaIgual(ultimaLin, ultimaCol, +1, 0) == 3;
    
       // horizontal -
    if (!result)
      result = contaIgual(ultimaLin, ultimaCol, 0, -1) + 1 + contaIgual(ultimaLin, ultimaCol, 0, +1) == 3;
       
       // diagonal \ 
    if (!result)
      result = contaIgual(ultimaLin, ultimaCol, -1, -1) + 1 + contaIgual(ultimaLin, ultimaCol, +1, +1) == 3;
  ...
  public void jogada(int lin, int col) {
    ...
    ultimoJogador = (ultimoJogador != JOGADOR1) ? JOGADOR1 : JOGADOR2;  // forma abreviada de fazer um if .. else
    ultimaLin = lin-1;
    ultimaCol = col-1;
